...
#most recent filter
    ...
    special_words = Words.objects.filter(kanji__in = new_kanji).exclude(published = False).exclude(id__in = known_word_list).exclude(frequency_thousand__gte = 21).prefetch_related('kanji').order_by('frequency_thousand')
    ...
    if len(special_words) == 0:        
        ...
        words = Words.objects.filter(kanji__in = kanji_in).exclude(id__in = known_word_list).exclude(published = False).exclude(frequency_thousand__gte = 21).order_by('frequency_thousand').prefetch_related('kanji').distinct()[0:1000]
