#execute this in django shell: execfile('xmltodict/pos-upload-two.py')
import xml.etree.ElementTree as ET
from manageset.models import UserProfile, Sets, Words, Kanji, WordMeanings, WordPos
        ...
        if the_word.exists():
            ...
            for each in sense.iter():
                ...
                for each in pos:
                    ...
                    # print WordPos.objects.filter(word = the_word_instance, pos = the_pos)
                    # print word_pos_check
                    if word_pos_check:
                        exist_count += 1
                        # print "pos already exists"
                    else:
                        new_pos = WordPos(word = the_word_instance, pos = the_pos)
                        new_pos.save()
                        non_exist_count+=1    
